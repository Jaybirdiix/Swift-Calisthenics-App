...
def main():
    ...
    # trivial greedy: warmup(2) -> skill(1) -> strength(2) -> accessory(1) -> cooldown(1)
    def pick(pred): 
        for e in bucket:
            if pred(e): return e
        return bucket[0]
    plan = {
      ...
      "blocks": [
        {"name":"warmup","items":[{"id":"e0","sets":1,"dose":"2-3m","notes":"pulse raiser + mobility"}]},
        {"name":"skill","items":[{"id":"e1","sets":3,"dose":"20s","notes":"form & control"}]},
        {"name":"strength","items":[{"id":"e2","sets":3,"dose":"8-10 reps","notes":"RPE 7"}, {"id":"e3","sets":3,"dose":"8-10 reps","notes":"full ROM"}]},
        {"name":"accessory","items":[{"id":"e4","sets":2,"dose":"12-15 reps","notes":"quality over speed"}]},
        {"name":"cooldown","items":[{"id":"e5","sets":1,"dose":"3-5m","notes":"breathe/relax"}]}
      ]
    }
    from pydantic import TypeAdapter
    plan = TypeAdapter(WorkoutPlan).validate_python(plan)
    with open(args.out, "w", encoding="utf-8") as f:
        f.write(plan.model_dump_json(indent=2))
    print(f"✅ Wrote {args.out} (deterministic)")
    return
