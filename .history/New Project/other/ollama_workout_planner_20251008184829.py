...
def format_plan(result: dict, fmt: str) -> str:
    ...
    if fmt == "text":
        ...
        for i, ex in enumerate(plan, 1):
            ...
            # one line per exercise; tweak to taste
            lines.append(f"{i}. {name}  {f'— {reps}' if reps else ''}  (difficulty {diff})")
        # optional: show focus scores
        fs = result.get("focus_scores", {})
        if fs:
            lines.append("\nFocus scores:")
            for m, s in sorted(fs.items(), key=lambda kv: (-kv[1], kv[0])):
                lines.append(f"• {m}: +{s}")
        return "\n".join(lines)
    if fmt == "markdown":
        out = ["| # | Exercise | Prescription | Difficulty |",
               "|---|----------|--------------|------------|"]
        for i, ex in enumerate(plan, 1):
            out.append(f"| {i} | {ex.get('name','')} | {ex.get('reps','')} | {ex.get('difficulty','')} |")
        return "\n".join(out)
    # fallback
    return json.dumps(result, indent=2, ensure_ascii=False)
...
def main():
    ...
    ap.add_argument(
        ...
        default="json",
        help="Output format: json (default), text (bullet list), or markdown (table)"
    )
    ...
    print(format_plan(result, args.format))
